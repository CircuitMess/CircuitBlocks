...
export default class InstallerLinux {
	checkInstall(): InstallInfo {
		...
		let arduinoLocal = path.join(os.homedir(), ".arduino15");
		if(!fs.existsSync(arduinoLocal)){
			arduinoLocal = path.join(os.homedir(), ".arduino");
		}
