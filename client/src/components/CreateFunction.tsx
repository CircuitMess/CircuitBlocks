...
> {
  ...
  modalDidOpen() {
    ...
    const functionBeingEdited = functionEditorWorkspace.newBlock(
        'function_declaration'
      ) as any /*Blockly.FunctionDeclarationBlock*/;
  ...
  componentDidUpdate(
    prevProps: Readonly<ISettingsProps>,
    prevState: Readonly<CreateFunctionDialogState>,
    snapshot?: any
  ): void {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <sui.Modal.Actions>
          {actions.map((action) => (
            <sui.Button className={action.className} onClick={action.onclick}>
              {action.label}
            </sui.Button>
          ))}
        </sui.Modal.Actions>
